...
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static Screen createConfigScreen(Screen parent) {
        ...
        builder.getOrCreateCategory(Component.empty()) // text will be ignored since it's the only category
            .addEntry(
                entryBuilder.startIntList(
                    Component.translatable("text.autoreconnect.config.option.delays"),
                    autoreconnect.config.AutoReconnectConfig.getInstance().delays
                ).setCreateNewInstance(
                    list -> new IntegerListListEntry.IntegerListCell(autoreconnect.config.AutoReconnectConfig.defaultDelay, list)
                ).setInsertInFront(false)
                .setMin(1)
                .setExpanded(true)
                .setDefaultValue(autoreconnect.config.AutoReconnectConfig.defaultDelays)
                .setSaveConsumer(
                    delays -> autoreconnect.config.AutoReconnectConfig.getInstance().delays = delays
                ).setTooltip(
                    Component.translatable("text.autoreconnect.config.tooltip.option.delays")
                ).build()
            ).addEntry(
                entryBuilder.startBooleanToggle(
                    Component.translatable("text.autoreconnect.config.option.infinite"),
                    autoreconnect.config.AutoReconnectConfig.getInstance().infinite
                ).setDefaultValue(autoreconnect.config.AutoReconnectConfig.defaultInfinite)
                .setTooltip(Component.translatable("text.autoreconnect.config.tooltip.option.infinite"))
                .setSaveConsumer(infinite -> autoreconnect.config.AutoReconnectConfig.getInstance().infinite = infinite)
                .build()
            ).addEntry(
                new NestedListListEntry<AutoMessages, MultiElementListEntry<AutoMessages>>(
                    Component.translatable("text.autoreconnect.config.option.automessages"),
                    autoreconnect.config.AutoReconnectConfig.getInstance().autoMessages,
                    true,
                    () -> Optional.of(new Component[]{Component.translatable("text.autoreconnect.config.tooltip.option.automessages")}),
                    list -> autoreconnect.config.AutoReconnectConfig.getInstance().autoMessages = list,
                    () -> autoreconnect.config.AutoReconnectConfig.defaultAutoMessages,
    ...
    private static MultiElementListEntry<AutoMessages> createAutoMessagesEntry(ConfigEntryBuilder entryBuilder, AutoMessages autoMessages) {
        ...
        tmp.setTooltipSupplier(
            () -> Optional.of(
                new Component[] { Component.translatable("text.autoreconnect.config.tooltip.option.automessages.instance") }
            )
        );
        return tmp;
    }
    ...
    private static Optional<Component> emptyListErrorSupplier(List<?> list) {
        return list == null
            || list.isEmpty()
                ? Optional.of(Component.translatable("text.autoreconnect.config.error.empty_list"))
                : Optional.empty();
    }
    ...
    private static Optional<Component> emptyStringErrorSupplier(String str) {
        return str == null
            || str.isEmpty()
                ? Optional.of(Component.translatable("text.autoreconnect.config.error.empty_string"))
                : Optional.empty();
    }
}
