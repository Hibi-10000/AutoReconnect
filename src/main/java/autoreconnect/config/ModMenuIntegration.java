...
public class ModMenuIntegration implements ModMenuApi {
        ...
        builder.getOrCreateCategory(Text.empty()) // text will be ignored since it's the only category
            .addEntry(
                entryBuilder.startIntList(
                    ...
                    list -> new IntegerListListEntry.IntegerListCell(AutoReconnectConfig.defaultDelay, list)
                ).setInsertInFront(false)
                .setMin(1)
                .setExpanded(true)
                .setDefaultValue(AutoReconnectConfig.defaultDelays)
                .setSaveConsumer(
                    delays -> AutoReconnectConfig.getInstance().delays = delays
                ).setTooltip(
                    Text.translatable("text.autoreconnect.config.tooltip.option.delays")
                ).build()
            ).addEntry(
    ...
    private static MultiElementListEntry<AutoMessages> createAutoMessagesEntry(ConfigEntryBuilder entryBuilder, AutoMessages autoMessages) {
        ...
        tmp.setTooltipSupplier(
            () -> Optional.of(
                new Text[] { Text.translatable("text.autoreconnect.config.tooltip.option.automessages.instance") }
            )
        );
        return tmp;
    }
    ...
    private static Optional<Text> emptyListErrorSupplier(List<?> list) {
        return list == null
            || list.isEmpty()
                ? Optional.of(Text.translatable("text.autoreconnect.config.error.empty_list"))
                : Optional.empty();
    }
    ...
    private static Optional<Text> emptyStringErrorSupplier(String str) {
        return str == null
            || str.isEmpty()
                ? Optional.of(Text.translatable("text.autoreconnect.config.error.empty_string"))
                : Optional.empty();
    }
}
